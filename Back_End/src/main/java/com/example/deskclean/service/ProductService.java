...
public class ProductService {
    ...
    // 상품 이미지 URL 업데이트
    @Transactional
    public void updateProductImage(Long productId, String username, String imageUrl) {
        Product product = productRepository.findById(productId)
                .orElseThrow(() -> new IllegalArgumentException("상품을 찾을 수 없습니다: " + productId));

        // 권한 확인: 판매자만 이미지 업로드 가능
        if (!product.getSeller().getUsername().equals(username)) {
            throw new IllegalArgumentException("상품 이미지를 수정할 권한이 없습니다.");
        }
